...
function FormField({ label, name, fieldId, type = 'text', value, placeholder, error, onChange, icon: Icon, helper }) {
  const helpId = `${fieldId}-help`
  const errorId = `${fieldId}-error`
  const describedBy = [helpId, error ? errorId : null].filter(Boolean).join(' ')

  return <div className="mb-5"><label htmlFor={fieldId} className="flex items-center gap-2 text-sm font-bold"><span className="text-brand"><Icon size={16} /></span>{label}</label><input id={fieldId} name={name} type={type} value={value} onChange={onChange} placeholder={placeholder} aria-invalid={Boolean(error)} aria-describedby={describedBy} className={`mt-2 min-h-12 w-full rounded-xl border bg-white px-3 text-sm text-ink outline-none transition placeholder:text-stone-400 focus-visible:border-brand focus-visible:ring-2 focus-visible:ring-brand/20 ${error ? 'border-red-500' : 'border-stone-200'}`} /><p id={helpId} className="mt-1.5 text-xs leading-5 text-muted">{helper}</p>{error && <p id={errorId} role="alert" className="mt-1 text-xs font-medium text-red-600">{error}</p>}</div>
}
